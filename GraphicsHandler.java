...
public class GraphicsHandler {
	...
	public GraphicsHandler(int radius, Workspace w) {
		this.w = w;
		this.radius = w.zoomHandler.divZ(radius);
	}
	...
	public static void circle(Graphics2D g,Node n, String type, int radius, Color c) {
		//System.out.println("drawing circle around node " + n);
		g.setColor(c);
		if(type == "fill")
	...
	public void drawProtoLandmas(Graphics2D g) {
		g.setColor(Color.RED);
		ArrayList<Node> n = w.protoLandmass;
		int count = n.size();
		
		for(int i = 1; i < count; i++) {
			g.drawLine(n.get(i-1).x(), n.get(i-1).y(), n.get(i).x(), n.get(i).y());
		}
			if(w.mouseInViewport) {
				g.drawLine(n.get(count-1).x(), n.get(count-1).y(), w.mouseNode().x(), w.mouseNode().y());
			}else {
